...
def get_json_from_gpt_response(data: str) -> Dict:
    ...
    start = data.find("```")
    # if directly responds as json with no additional text
    if start == -1:
...
def get_role_content(val: int) -> str:
    ...
    if val == 0:
        # for code info
        return """You are a code summarizer which return code info precisely to be read by gpt-3.5 model."""
    elif val == 1:
        # for inti info
        return """You are a software developer which read code summary generated by a gpt-3.5 model."""
    elif val == 2:
        # for chapter name
        return """You are gpt model which responds in json."""
    elif val == 3:
        # for chapter contents
        return """You are a software engineering teacher."""
    elif val == 4:
        # for project intro
...
def get_prompt_for_code_info() -> str:
    ...
    return """Offer a comprehensive summary that encapsulates the core functionalities of the code provided below.
    Include all important information (literals like endpoints and other things).
    Also take account into the package or module the code corresponds to using the paths given.
    Limit your response to a maximum of 400 characters (including spaces).
    Path: {0}
    Code: {1}
    """
...
def get_prompt_for_chapters_name() -> str:
    # Consider the codebase as a whole and analyze the structural design of the system and give the chapter for better understanding of the architecture.
...
def get_prompt_for_chapter_contents() -> str:
    return """"Produce the contents of the chapter of a software project given the project purpose and the 
    files associated with the chapter.
    Chapter name: {0}
    Purpose: {1}
    Files: {2}
    """
